 ...
 * and calculating payment summaries using MongoDB aggregation.
 * Integrates with an external random number API to simulate payment gateway logic.
 */
public interface PaymentService {
   ...
   * then asynchronously calls external random number API to determine
   * final status (SUCCESS if even, FAILED if odd).
   *
   ...
   * @throws com.innowise.paymentservice.exception.PaymentNotFoundException       if payment not found
   * @throws com.innowise.paymentservice.exception.PaymentAccessDeniedException   if USER tries to access another user's payment
   */
